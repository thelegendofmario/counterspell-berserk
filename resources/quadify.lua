    ...
    for i = 0, quad.image_columns - 1 do
        for j = 0, quad.image_rows - 1 do
            table.insert(quad.quads,
                love.graphics.newQuad(1 + j * (quad.twidth + 2), 1 + i * (quad.theight + 2), quad.twidth, quad.theight,
                    quad.image_width, quad.image_height))
        end
    end
end
    ...
    for i, row in ipairs(tilemap) do
        for j, tile in ipairs(row) do
            if tile ~= 0 then
                ...
                love.graphics.draw(quad.base_image, quad.quads[tile], j * quad.twidth - quad.twidth,
                    i * quad.theight - quad.theight)
            end
        end
    end
end
